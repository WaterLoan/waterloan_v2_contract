...
export const initIncentiveController = async (rewardTokenAddress: string, admin: string) => {
  // Deploy vault
  // Need to transfer initial amount of reward tokens to it
  const vault = await deployWatIncentivesVault([rewardTokenAddress]);
  const REWARD = await getIErc20Detailed(rewardTokenAddress);
  const amount = utils.parseEther('3000000');
  await REWARD.transfer(vault.address, amount);

  // Deploy incentives controller
  const controller = await deployTokenIncentivesController([rewardTokenAddress, vault.address, admin]);
  await vault.approve(controller.address, amount);
