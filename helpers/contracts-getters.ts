...
export const getAllMockedTokens = async () => {
  ...
  const tokens: MockTokenMap = await Object.keys(TokenContractId).reduce<Promise<MockTokenMap>>(
    async (acc, tokenSymbol) => {
      const accumulator = await acc;
      const tokenEntry = db.get(`${tokenSymbol.toUpperCase()}.${DRE.network.name}`).value();
...
export const getTokenIncentivesController = async (address?: tEthereumAddress) =>
  await TokenIncentivesControllerFactory.connect(
    address ||
      (await getDb().get(`${eContractid.TokenIncentivesController}.${DRE.network.name}`).value())
        .address,
    await getFirstSigner()
  );

export const getWatIncentivesVault = async (address?: tEthereumAddress) =>
  await WatIncentivesVaultFactory.connect(
    address ||
      (await getDb().get(`${eContractid.WatIncentivesVault}.${DRE.network.name}`).value())
        .address,
    await getFirstSigner()
  );

export const getStakedTokenV3 = async (address?: tEthereumAddress) =>
  await StakedTokenV3Factory.connect(
    address ||
...
export const getDistributionManager = async (address: tEthereumAddress) =>
  await DistributionManagerFactory.connect(
    address,
    await getFirstSigner()
  );