...
import {
  ...
  TokenIncentivesControllerFactory,
  WatIncentivesVaultFactory,
  StakedTokenV3Factory,
} from '../types';
import {
  withSaveAndVerify,
...
export const deployAllMockTokens = async (verify?: boolean) => {
  ...
  const protoConfigData = getReservesConfigByPool(AavePools.waterloan);
  ...
  for (const tokenSymbol of Object.keys(TokenContractId)) {
    ...
    if (!configData && tokenSymbol != TokenContractId.USD) {
      continue;
    }
...
export const deployWCETMocked = async (verify?: boolean) =>
  withSaveAndVerify(
    await new WCET9MockedFactory(await getFirstSigner()).deploy(),
    eContractid.WCETMocked,
    [],
    verify
  );
...
export const deployTokenIncentivesController = async (
  args: [tEthereumAddress, tEthereumAddress, tEthereumAddress],
  verify?: boolean
) =>
  withSaveAndVerify(
    await new TokenIncentivesControllerFactory(await getFirstSigner()).deploy(...args),
    eContractid.TokenIncentivesController,
    args,
    verify
  );

export const deployWatIncentivesVault = async (
  args: [tEthereumAddress],
  verify?: boolean
) =>
  withSaveAndVerify(
    await new WatIncentivesVaultFactory(await getFirstSigner()).deploy(...args),
    eContractid.WatIncentivesVault,
    args,
    verify
  );

export const deployStakedTokenV3 = async (
  args: [tEthereumAddress, tEthereumAddress, string, string, tEthereumAddress, tEthereumAddress, string, string, string],
  verify?: boolean
) =>
  withSaveAndVerify(
    await new StakedTokenV3Factory(await getFirstSigner()).deploy(...args),
