  ...
  .setAction(async ({ verify, pool }, DRE) => {
    try {
      ...
      if (notFalsyOrZeroAddress(aaveOracleAddress)) {
        ...
        let wethAddress = await getWethAddress(poolConfig);

        if (network == eEthereumNetwork.coinexTest || network == eEthereumNetwork.coinex) {
          console.log("Avoiding AaveOracle to handle WCET as WETH which return 1 ether price.");
          wethAddress = ZERO_ADDRESS;
        }

        aaveOracle = await deployAaveOracle(
          [tokens, aggregators, fallbackOracleAddress, wethAddress],
          verify
        );
