...
makeSuite('Pausable Pool', (testEnv: TestEnv) => {
  ...
  it('Flash loan', async () => {
    ...
    // Pause pool
    await configurator.connect(users[1].signer).setPoolPause(true);
    await configurator.connect(users[1].signer).setFlashLoanPause(true);
    ...
    ).revertedWith(LP_FLASHLOAN_IS_PAUSED);

    // Unpause flashLoan
    await configurator.connect(users[1].signer).setFlashLoanPause(false);
